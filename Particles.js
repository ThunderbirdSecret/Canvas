...
const cnv = document.querySelector('canvas');//настраиваем canvas
const ctx = cnv.getContext('2d'); //указываем в каком контексте работаем

const cfg = {
orbsCount   : 400, //увеличиваем колличество сфер, для их хранения orbsList[]
minVelocity : .5, //min скорость
ringsCount  : 10, //кол-во колец по которым двигаются частицы
}

let mx = 0, my = 0;// получаем координаты мыши
cnv.addEventListener(`mousemove`, e => {// слушатель событий переключения мыши на canvas
...
function resize() {
  ...
  cx = cw * .5;
  cy = ch * .5;
  ph = cy * .4;//радиус пустоты в центре, чуть меньше высоты области просмотра
  pw = cx * .4;// часть ширины, для формирования орбиты овала
}
...
class Orb {     // создаем объект и задаем его св-ва
    constructor() {
      this.size     = Math.random() * 5 + 2;
      this.angle    = Math.random() * 360;//свойство градусa ot 0 do 360
      this.radius   = (Math.random() * cfg.ringsCount | 0) * ph / cfg.ringsCount;// | округление с помощью битовой операции(альтернатива методу floor)округляет число до целого в меньшую сторону здесь от 0 до 10
      ...
      this.velocity = cfg.minVelocity + Math.random() * cfg.minVelocity - this.impact;//передвижение частиц, рандом для разной скорости каждой
    }
    
    refresh() { //метод который вызваем каждый кадр для движения и прорисовки сфер
      ...
      let paralaxX = mx / cw * 2 - 1;//позиция_мыши / ширина_холста, используем для наклона. за расположение по высоте отвечает Y
      let paralaxY = my / ch;// смещение вверх при позиции курсора


      let x       = cx + cos * (ph + this.radius) + offsetX;//центр х, растянуть
      let y       = cy + sin * (ph + this.radius) - offsetY * paralaxY - paralaxX * offsetX; //сплюснуть
      ...
      let optic   = sin * this.size * this.impact * .7; // увеличиваем сферы нижние, чтобы они казались ближе| точки ближе к центру подвергаются минимальным изменениям р-ра
      ...
      if (distToC > ph - 1 || sin > 0) { //не отрисовываем частицы за сферой
        ctx.strokeStyle = ctx.fillStyle = color;
        ctx.beginPath(); //откроем путь
        ctx.arc(x, y, size, 0, 2 * Math.PI);// рисуем круг// заменили this.size на size
        distToM <= 50 ? ctx.stroke() : ctx.fill();//заливка
      }
      
      this.angle = (this.angle - this.velocity) % 360;//меням значение угла +||- значение скорость, если менять, то изменится направление. В зависимости от знака движение по часовой стрелке или проти
  ...
  function createStardust(){ //наполняем orbsList через ф-цию
  ...
  function loop(){ // отобразить каждый элемент orbsList
